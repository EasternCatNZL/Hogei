...
public class CactusRandomSprayBOSS : MonoBehaviour {
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (percentLoss < NextRampTrigger)
        {
            ...
            {
                float RandNum = Random.Range(0f, Range) - Range / 2;
                Instantiate(PulseObject, transform.position + new Vector3(RandNum, 0f, RandNum), transform.rotation);
                print((transform.position + new Vector3(RandNum, 0f, RandNum)).ToString());
            }
            numPulses += 1;
        }
    }
